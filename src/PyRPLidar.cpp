...
PYBIND11_MODULE(PyRPLidar, m)
{
    ...
    py_lidar.def("get_scanline", 
        [](PyLidar& self, bool filter_quality){
            auto data = self.get_scan_as_lidar_samples(filter_quality);

            lidar_sample* samples = data.first.release();
